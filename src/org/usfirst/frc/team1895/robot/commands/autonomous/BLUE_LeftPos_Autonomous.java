 ...
 * ON THE LEFT
 */
public class BLUE_LeftPos_Autonomous extends CommandGroup {

    public BLUE_LeftPos_Autonomous() {
    	
    	//UPDATED WITH RETRY CODE
    	
    	addSequential(new DriveStraightSetDistance(-83)); //117
    	//TURN TOWARD TO AIRSHIP'S LIFT
    	addSequential(new TurnWithoutPID(45, 0.35)); //this angle depends on where we are on the field
    	...
    	addSequential(new TurnWithoutPID(-45.0, 0.35));
    	//DRIVE FORWARD INTO NEUTRAL ZONE
    	addSequential(new DriveStraightSetDistance(-60));	
    	addSequential(new TurnWithoutPID(45.0, 0.35));		//Mr.Brey wants us to drive toward the boiler but stay in NZ

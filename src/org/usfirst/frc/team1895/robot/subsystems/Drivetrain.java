...
public class Drivetrain extends Subsystem {
	...
	// Instance variables. There should only be one instance of Drivetrain, but we
	// are assuming the programmer will not accidently create multiple instances
...
//	// private CANTalon right_motor3;
	...
	private static final double TANK_DRIVE_SCALAR = .97;

	// Motors (not motorgroups!)
	...
	int printCounter = 0;
	/*
	 * raise P constant until controller oscillates. If oscillation too much, lower
	 * constant a bit raise D constant to damp oscillation, causing it to converge.
	 * D also slows controller's approach to setpoint so will need to tweak balance
	 * of P and D if P + D are tuned and it oscillates + converges, but not to
	 * correct setpoint, increase I = steady-state error - positive, nonzero
	 * integral constant will cause controller to correct for it
	 */
	...
	public Drivetrain() {
		...
		// Digital IO
		l_encoder = new Encoder(RobotMap.LEFT_GEARBOX_ENCODER_A_PORT, RobotMap.LEFT_GEARBOX_ENCODER_B_PORT, true,
				EncodingType.k4X);
		r_encoder = new Encoder(RobotMap.RIGHT_GEARBOX_ENCODER_A_PORT, RobotMap.RIGHT_GEARBOX_ENCODER_B_PORT, true,
				EncodingType.k4X);
		...
		pidControllerDriving = new PIDController(pGainDriv, iGainTurn, dGainDriv, l_encoder, myPIDOutputDriving); // Input
																														...
																														// ,
																														// output
		// pidControllerTurning = new PIDController(pGainTurn, iGainTurn, dGainTurn,
		// gyro, myPIDOutputTurning);
		pidControllerTurning = new PIDController(pGainTurn, iGainTurn, dGainTurn, ahrs, myPIDOutputTurning);

		// Solenoids
		// left_solenoid = new DoubleSolenoid(RobotMap.L_DRIVETRAIN_SOLENOID_A_PORT,
		// RobotMap.L_DRIVETRAIN_SOLENOID_B_PORT);
		// right_solenoid = new DoubleSolenoid(RobotMap.R_DRIVETRAIN_SOLENOID_A_PORT,
		...
		// smaller = farther
		l_encoder.setDistancePerPulse(0.0225);
		r_encoder.setDistancePerPulse(0.0225);
	...
	// Sensors: None
	// Description: A basic tank drive method. The two parameters are expected to be
	...
	// For: DefaultDrive Command
	// Sensors: None
	// Description: A basic arcade drive method. The two parameters are expected to
	// be within the range -1.0 to 1.0
	// If not, they are limited to be within that range. The transitional speed and
	// yaw are combined
	// to be applied to the left motor and right motor. Trans_speed (transitional
	// velocity) will
	// set the robot's forward speed, and yaw (angular velocity) will set the robot
	// turning. Having a
	...
	public void arcadeDrive(double trans_speed, double yaw) {
		...
		*/
		...
		// The scalar values were determined through testing that our left motors
		// were outputting more power than the right.

		if (trans_speed > 0) { // forward
			left_dt_motor1.set(ControlMode.PercentOutput, left_speed * .985); // 
...
//		System.out.println("Driving L " + left_speed + " R " + right_speed);
		System.out.println("Left1: " + left_dt_motor1.getOutputCurrent() + " Left2: " + left_dt_motor2.getOutputCurrent() + " Right1: " + right_dt_motor1.getOutputCurrent() + " Right2: " + right_dt_motor1.getOutputCurrent());
...
//			System.out.println("Got invalid PID output for driving");
	...
	public boolean turnWithPID(double desiredTurnAngle) {
		...
		// basicArcadeDrive uses x, y inputs so it should be 0 for y and whatever the
		// PIDcontroller calculates as x
...
//					l_encoder.getDistance(), r_encoder.getDistance(), pidControllerTurning.getSetpoint(),
...
//	}
	...
	// Description: Use the 3 rangefinders to get the distance the front two corners
	// and the front of the robot are from
	// the boiler. If the left and right corners are the same distance (within a
	// certain margin of error) and exceed a certain
	// distance, and if the middle rangefinder exceeds a certain threshold distance,
	// then the robot knows it had reached its
	// final position.
...
//		double right_speed = speed;
		if (fineDistanceFinder() <= (goaldistance)) {// if the robot crossed the goal distance + buffer then the code
														// will stop
			arcadeDrive(0, 0);
	...
	// Sensors: Encoders
	/**
	 * This method should check to see if the left or right encoder read a value o
	 * greater than 4 ft/s. If so, shift nto high gear. This will avoid the large
	 * acceleration time required for high gear. if less than 3.7 ft/s, shift back
	 * into low gear.
	 */
	public void shiftGears() {
		// Disabling shifting for the rest of competition
		// If Teleop is not enabled, skip. Autonomous doesn't work with shifting
		if (!teleopEnabled)
			return;
		...
		double max = Math.max(Math.abs(l_encoder.getRate()), Math.abs(r_encoder.getRate()));
	...
	/**
	 * This method is used to enable or diable manual control. If the first param is
	 * false, the second param has no effect.
	 * 
...
//	}
	...
	public void makeNewPidDriving(double p, double i, double d) {
		//// ensures everytime we make a new PID controller, we have a fresh output
		myPIDOutputDriving = new MyPIDOutput();
...
//		System.out.println("Left encoder: " + l_encoder.getDistance());
//		System.out.println("Right encoderL " + r_encoder.getDistance());
//
//		System.out.println("\nLM1_cur: " + left_dt_motor1.getOutputCurrent());
//		System.out.println("LM2_cur: " + left_dt_motor2.getOutputCurrent());
//
//		System.out.println("RM1_cur: " + right_dt_motor1.getOutputCurrent());
//		System.out.println("RM2_cur: " + right_dt_motor2.getOutputCurrent());
//
...
//		System.out.println("Joy X" + Robot.oi.gamepad.getAxis(F310.LX));
	...
	public boolean driveWithEncoders(double v, double distancetoTravel) {
		...
		double l_encoderDistance = l_encoder.getDistance();
		double r_encoderDistance = r_encoder.getDistance();
	...
	// Quick note: whatever value you want, subtract 10
	...
	// "Thar be dragons when motors on the same gearbox are set differently" (Scott
	// 2017), so
	// a MotorGroup will handle setting multiple motors to the same value as if it
	// were one motor.
